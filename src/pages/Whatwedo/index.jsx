...
const keyframes = {
    ...
    '@keyframes wd_shimmer': { from: { left: '-100%' }, to: { left: '100%' } },
};
...
const PROGRAMS = [
    { id: 'crusades', icon: CampaignIcon, color: colors.secondary.main, title: 'Crusades', tagline: 'Gospel & revival outreaches — Word, Works, and Worth in every community.' },
    { id: 'widows', icon: VolunteerActivismIcon, color: colors.accent.red, title: 'Widows & Orphans Support', tagline: 'Practical, sustained care that restores dignity to the most vulnerable.' },
    { id: 'ministers', icon: PeopleAltIcon, color: '#8B5CF6', title: "Ministers & Ministers' Children Support", tagline: 'Standing with pastors and families who serve under difficult conditions.' },
    { id: 'gcem', icon: MenuBookIcon, color: colors.accent.teal, title: 'Grace College of Evangelism & Missions', tagline: 'Raising the next generation of evangelists through rigorous biblical training.' },
];
...
// Five mandate points — just for the teaser icons
const MANDATE_ICONS = [
    { Icon: CampaignIcon, color: colors.secondary.main },
    { Icon: HandshakeIcon, color: colors.accent.teal },
    { Icon: VolunteerActivismIcon, color: colors.accent.red },
    { Icon: MenuBookIcon, color: '#8B5CF6' },
    { Icon: LocalHospitalIcon, color: colors.accent.green },
];

export default function WhatWeDoPage() {
    const [heroRef, heroVis] = useReveal(0.05);
    const [statsRef, statsVis] = useReveal();
    const [mandateTeaserRef, mandateTeaserVis] = useReveal();
    ...
    return (
        <>
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: { xs: 8, md: 6 }, alignItems: 'center' }}>
                        <Box ref={heroRef}>
                            <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 1, bgcolor: 'rgba(212,160,23,0.1)', border: '1px solid rgba(212,160,23,0.28)', borderRadius: '100px', px: 2.2, py: .75, mb: 3, opacity: heroVis ? 1 : 0, animation: heroVis ? 'wd_rise .6s ease both' : 'none' }}>
                                ...
                                <Typography sx={{ fontFamily: typography.fontFamily.body, fontSize: '0.65rem', fontWeight: 700, color: colors.secondary.light, letterSpacing: 2.5, textTransform: 'uppercase' }}>Our Work</Typography>
                            </Box>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', opacity: heroVis ? 1 : 0, animation: heroVis ? 'wd_rise .9s cubic-bezier(.34,1.2,.64,1) .3s both' : 'none' }}>
            ...
            {/* ══ MANDATE TEASER — beautiful banner card ══ */}
            <Box ref={mandateTeaserRef} sx={{ bgcolor: colors.background.default, pb: { xs: 8, md: 10 }, px: { xs: 2, md: 0 } }}>
                <Container maxWidth="lg">
                    <Box
                        component={Link}
                        to="/whatwedo/mandate"
                        sx={{
                            ...
                            overflow: 'hidden',
                            position: 'relative',
                            background: `linear-gradient(135deg, ${colors.primary.dark} 0%, #0F2744 50%, ${colors.primary.dark} 100%)`,
                            border: `1px solid rgba(212,160,23,0.2)`,
                            p: { xs: 4, md: 6 },
                            opacity: mandateTeaserVis ? 1 : 0,
                            animation: mandateTeaserVis ? 'wd_rise .8s ease both' : 'none',
                            transition: 'transform .35s ease, box-shadow .35s ease',
                            '&:hover': {
                                transform: 'translateY(-6px)',
                                boxShadow: `0 32px 80px rgba(5,10,30,0.35), 0 0 0 1px rgba(212,160,23,0.35)`,
                            },
                            // shimmer on hover
                            '&::after': {
                                content: '""',
                            ...
                            '&:hover::after': { animation: 'wd_shimmer .7s ease forwards' },
                        }}
                    >
                        {/* Dot grid bg */}
                        <Box sx={{ position: 'absolute', inset: 0, backgroundImage: `radial-gradient(rgba(212,160,23,0.06) 1px, transparent 1px)`, backgroundSize: '24px 24px', pointerEvents: 'none' }} />
                        {/* Glow orbs */}
                        <Box sx={{ position: 'absolute', top: '-30%', right: '-5%', width: 380, height: 380, borderRadius: '50%', background: `radial-gradient(circle,${colors.secondary.main}14 0%,transparent 70%)`, pointerEvents: 'none' }} />
                        <Box sx={{ position: 'absolute', bottom: '-40%', left: '20%', width: 280, height: 280, borderRadius: '50%', background: `radial-gradient(circle,${colors.accent.teal}0E 0%,transparent 70%)`, pointerEvents: 'none' }} />

                        <Box sx={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: { xs: 'flex-start', md: 'center' }, justifyContent: 'space-between', gap: { xs: 4, md: 6 } }}>
                            {/* Left — label + heading + sub */}
                            <Box sx={{ flex: 1 }}>
                                <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 1.2, bgcolor: 'rgba(212,160,23,0.12)', border: '1px solid rgba(212,160,23,0.28)', borderRadius: '100px', px: 2, py: .65, mb: 2.5 }}>
                                    <Box sx={{ width: 5, height: 5, borderRadius: '50%', bgcolor: colors.secondary.main }} />
                                    <Typography sx={{ fontFamily: typography.fontFamily.body, fontSize: '0.6rem', fontWeight: 700, color: colors.secondary.light, letterSpacing: 2.5, textTransform: 'uppercase' }}>
                                        Our Foundation
                                    </Typography>
                                </Box>
                                <Typography sx={{ fontFamily: typography.fontFamily.accent, fontSize: { xs: '1.8rem', md: '2.6rem' }, fontWeight: 900, color: 'white', lineHeight: 1.1, mb: 1.5 }}>
                                    Read Our Mandate
                                </Typography>
                                <Typography sx={{ fontFamily: typography.fontFamily.body, fontSize: { xs: '0.9rem', md: '1rem' }, color: 'rgba(255,255,255,0.5)', lineHeight: 1.85, maxWidth: 480 }}>
                                    Five clear commitments that define everything Jesus Partners Outreach does — from preaching the Gospel to caring for the vulnerable.
                                </Typography>

                                {/* Five icon row */}
                                <Box sx={{ display: 'flex', gap: 1.5, mt: 3.5, flexWrap: 'wrap' }}>
                                    {MANDATE_ICONS.map(({ Icon, color }, i) => (
                                        <Box key={i} sx={{ width: 40, height: 40, borderRadius: '12px', bgcolor: `${color}16`, border: `1.5px solid ${color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            ...
                            {/* Right — CTA pill */}
                            <Box sx={{ flexShrink: 0 }}>
                                <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 2, bgcolor: colors.secondary.main, color: 'white', px: { xs: 3, md: 4 }, py: { xs: 1.6, md: 2 }, borderRadius: '14px', fontFamily: typography.fontFamily.heading, fontSize: { xs: '0.9rem', md: '1rem' }, fontWeight: 800, boxShadow: `0 12px 36px ${colors.secondary.main}44`, transition: 'all .3s ease' }}>
                                    See Our Mandate
                ...
                <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1 }}>
                    <Box sx={{ textAlign: 'center', mb: { xs: 7, md: 10 }, opacity: progsVis ? 1 : 0, animation: progsVis ? 'wd_rise .6s ease both' : 'none' }}>
                        ...
                        <Typography sx={{ fontFamily: typography.fontFamily.body, fontSize: typography.fontSize.base, color: colors.text.secondary, maxWidth: 500, mx: 'auto', lineHeight: 1.85 }}>
                            Click any program to read the full detail on its own page.
                        </Typography>
                    </Box>
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: { xs: 3, md: 4 } }}>
                        {PROGRAMS.map((prog, i) => {
                            ...
                            return (
                                <Box key={prog.id} sx={{ bgcolor: colors.background.default, borderRadius: '22px', border: `1px solid ${colors.divider}`, display: 'flex', flexDirection: 'column', overflow: 'hidden', opacity: progsVis ? 1 : 0, animation: progsVis ? `wd_rise .7s ease ${i * .1}s both` : 'none', transition: 'transform .3s ease, box-shadow .3s', '&:hover': { transform: 'translateY(-6px)', boxShadow: `0 24px 60px ${prog.color}1A` } }}>
                                    <Box sx={{ height: 4, background: `linear-gradient(90deg, ${prog.color}, ${prog.color}66)` }} />
                                    <Box sx={{ p: { xs: 3, md: 4 }, display: 'flex', flexDirection: 'column', flex: 1 }}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                                            <Box sx={{ width: 52, height: 52, borderRadius: '16px', bgcolor: `${prog.color}14`, border: `1.5px solid ${prog.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                                <Icon sx={{ fontSize: 26, color: prog.color }} />
                                            </Box>
                                            <Typography sx={{ fontFamily: typography.fontFamily.accent, fontSize: { xs: '1.1rem', md: '1.25rem' }, fontWeight: 900, color: colors.text.primary, lineHeight: 1.2 }}>
                                                {prog.title}
                                            </Typography>
                                        </Box>
                                        <Typography sx={{ fontFamily: typography.fontFamily.body, fontSize: typography.fontSize.sm, color: colors.text.secondary, lineHeight: 1.75, mb: 3, flex: 1 }}>
                                            {prog.tagline}
                                        </Typography>
                                        <Box component={Link} to={`/whatwedo/${prog.id}`} sx={{ display: 'inline-flex', alignItems: 'center', gap: 1, alignSelf: 'flex-start', bgcolor: `${prog.color}12`, color: prog.color, border: `1.5px solid ${prog.color}35`, px: 2.5, py: 1.1, borderRadius: '10px', textDecoration: 'none', fontFamily: typography.fontFamily.heading, fontSize: '0.82rem', fontWeight: 800, transition: 'all .25s ease', '&:hover': { bgcolor: prog.color, color: 'white' } }}>
            ...
            <Box ref={ctaRef} sx={{ background: `linear-gradient(120deg,${colors.primary.dark} 0%,#0F3460 55%,${colors.primary.light} 100%)`, py: { xs: 8, md: 11 }, position: 'relative', overflow: 'hidden' }}>
                ...
                <Container maxWidth="md" sx={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
                    <Box sx={{ opacity: ctaVis ? 1 : 0, animation: ctaVis ? 'wd_rise .7s ease both' : 'none' }}>
                        <Typography sx={{ fontFamily: typography.fontFamily.accent, fontSize: { xs: '1.9rem', md: '2.6rem' }, fontWeight: 900, color: 'white', lineHeight: 1.12, mb: 1.5 }}>Join Us in the Field</Typography>
